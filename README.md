...
```bash
npm test                # 离线单测 120 项(加密/格式/库/渲染/Worker/端到端/弹窗/会话/同步/ZIP/资源清单/部署配置),Node ≥ 22
npm run test:falsify    # 鉴伪:逐条破坏 33 个守卫 → 确认对应用例真的会变红 → 自动还原
npx wrangler dev        # 本地真实 Worker + R2 模拟
```

⚠️ **改了 `public/` 下的任何东西,必须把 `public/sw.js` 里的 `CACHE` 版本号 +1。**
本 SW 是 cache-first 的,而浏览器只在 `sw.js` 这个文件本身变化时才重新安装它 ——
不升版本号,老用户会永远停在旧缓存里的 js 上,表现为「部署了但界面没变」。
新增 js 模块还要同步登记进 `ASSETS`(漏了会离线白屏);后者由 `tests/assets.test.mjs` 自动核对。
